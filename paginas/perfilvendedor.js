document.addEventListener('DOMContentLoaded', () => {
      ...
      productsToShow.forEach(p => {
          try {
              ...
              productDiv.innerHTML = `
                  ...
                  ${p.Status !== "Eliminado" ?
                  `
                  <form class="product-actions-form" method="post" action="actualizarproducto.php">
                      <input type="hidden" name="id_producto" value="${p.ID}"/>

                      <div class="stock-controls">
                          <button type="button" class="botones btnmenos" > - </button>
                          <input type="number" name="cantidadstock" class="cantidadstock" value="0" min="0"/>
              ...
              if (btnMenos && btnMas && inputCantidad) {
                  ...
                  btnMas.addEventListener("click", function (event) {
                      event.preventDefault(); // Prevenir el envío de la forma
                      let valorActual = parseInt(inputCantidad.value, 10);
                      inputCantidad.value = valorActual + 1;
                  });
              }
  ...
  function verificarMensajesPendientes() {
      fetch('get_mensajes.php')
          ...
          .then(data => {
              console.log("Arreglo de mensajes:", data.mensajes); 
  ...
  function abrirChat(chatId) {
      ...
      window.chatRefreshInterval = setInterval(refreshChatMessages, 1000);

      fetch(`get_producto_chat.php?id_chat=${chatId}`)
          .then(response => response.json())
          .then(data => {
              if (data.success) {
                  chatBody.dataset.productId = data.producto.id;
              } else {
  ...
  function refreshChatMessages() {
      ...
      fetch(`get_mensajes_chat.php?id_chat=${chatId}`)
          ...
          .then(data => {
              ...
              const isScrolledToBottom = chatBody.scrollHeight - chatBody.clientHeight <= currentScroll + 1; // +1 para margen
              ...
              if (data.mensajes.length === 0) {
                  ...
                  data.mensajes.forEach(msg => {
                      ...
                      mensajeElement.className = `chat-message ${msg.es_usuario ? 'user' : 'other'}`;
  ...
  cotizacionBtn.addEventListener('click', function () {
      const productId = chatBody.dataset.productId; 
      ...
      if (!productId) {
          ...
          alert("No se pudo cargar el producto para cotizar. Por favor, reinicia el chat.");
      ...
      fetch(`get_product_details.php?id=${productId}`)
          ...
          .then(data => {
              if (data.success) {
                  ...
                  cotizacionPrecio.value = parseFloat(data.product.precio) || '';
                  cotizacionDetalles.value = data.product.descripcion || '';
              } else {
                  console.error("Error al obtener detalles del producto:", data.error);
                  alert("Error al cargar detalles del producto para la cotización.");
                  cotizacionModal.classList.remove('visible');
                  container.classList.remove('visible'); 
                  container.classList.add('hidden');
              }
          ...
          .catch(error => {
              ...
              alert('Error de red al cargar los detalles del producto.');
              cotizacionModal.classList.remove('visible');
              container.classList.remove('visible'); 
              container.classList.add('hidden');
          });
  ...
  cotizacionEnviar.addEventListener('click', function () {
      ...
      if (!productId) {
          console.error("Error: No se encontró el ID del producto asociado al chat.");
          alert("Error: No se pudo identificar el producto para la cotización.");
          return;
      }

      const nombreProductoCotizacion = document.getElementById('cotizacion-producto').textContent; 
      const precioCotizacion = parseFloat(cotizacionPrecio.value);
      const detallesCotizacion = cotizacionDetalles.value.trim();

      if (isNaN(precioCotizacion) || precioCotizacion <= 0) {
      ...
      const mensajeCotizacion = `Cotización para ${nombreProductoCotizacion}:
Precio: ${formatCurrency(precioCotizacion)}
Detalles: ${detallesCotizacion}`;

      sendMessage(chatId, mensajeCotizacion);
      fetch('actualizar_producto_cotizacion.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: `id_producto=${encodeURIComponent(productId)}&precio=${encodeURIComponent(precioCotizacion)}&detalles=${encodeURIComponent(detallesCotizacion)}`
      })
          .then(response => response.json())
          .then(data => {
              if (data.success) {
                  alert("Producto actualizado a tipo 'Venta' con la cotización.");
                  reloadProducts();
              } else {
          ...
          .catch(error => {
              console.error('Error al enviar actualización de producto:', error);
              alert('Hubo un problema al actualizar el producto.');
          });

      document.getElementById('cotizacion-modal').classList.remove('visible');
      document.querySelector('.modal-cotizacion-container').classList.remove('visible'); 
      document.querySelector('.modal-cotizacion-container').classList.add('hidden');

      cotizacionPrecio.value = '';
